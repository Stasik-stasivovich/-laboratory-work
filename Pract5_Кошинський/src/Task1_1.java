...
public class Task1_1 extends GraphicsProgram {
	...
	private void createClouds() {
		...
		createRain(100,250,850,1050);
	}
	//робим дощ
	private void createRain(int x1, int x2, int x3, int x4) {
		ArrayList<GOval> rain = new ArrayList<GOval>();
		ArrayList<GOval> forDelete = new ArrayList<GOval>();
		Random q = new Random();
		while (true){
			for(int i=0;i<rain.size();i++){
				rain.get(i).move(0, 25);
				if (rain.get(i).getY()>420){
					remove(rain.get(i));
					forDelete.add(rain.get(i));
				}
			}
			for (GOval i:forDelete){
				rain.remove(i);
			}
			forDelete.clear();
			for(int i=0;i<8;i++){
				...
				GOval drop = new GOval (x+q.nextInt(80)-10,200,10,15);
				drop.setColor(Color.CYAN);
				drop.setFilled(true);
				add(drop);
				rain.add(drop);
			}
